...
class _ClientChatScreenState extends State<ClientChatScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (args != null) {
      ...
      _craftsmanName = args['craftsmanName'] ?? "الحرفي";
      _initUser();
    } else {
  ...
  Widget build(BuildContext context) {
    if (_currentUserId == null) {
      return Scaffold(
        appBar: AppBar(title: Text(_craftsmanName ?? "الدردشة")),
        body: const Center(child: CircularProgressIndicator()),
      );
    }
    return Scaffold(
      appBar: AppBar(title: Text(_craftsmanName!), backgroundColor: AppColors.primaryDarkBlue),
      body: Column(
        children: [
          Expanded(
